...
def request_foreign_st(st_sak_info):
    ...
    saml_aai_kerberos_socket = socket.socket(
        socket.AF_INET, socket.SOCK_STREAM)  # SSH socket
    saml_aai_kerberos_socket.connect(
        (SAML_AAI_KERBEROS_HOST, SAML_AAI_KERBEROS_PORT))
...
def ssh_foreign(st_info):
    ssh_foreign_socket = socket.socket(
        socket.AF_INET, socket.SOCK_STREAM)  # SSH socket
