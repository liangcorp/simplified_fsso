...
class ClientThread (threading.Thread):
    ...
    def run(self):
        ...
        if key_info_list[3] == st:
            ...
            self.conn.send('home remote shell~>')
...
SSHD_HOST = "127.0.0.1" # Get SSH Server Hostname
SSHD_PORT = 2224 # Port to simuate ssh server

KRB_HOST = "127.0.0.1" # Get Kerberos key Distribution Centre Hostname
KRB_PORT = 50002 # The KDC simulation's port number

def sshd_sim():
    try:
        s = socket.socket (socket.AF_INET, socket.SOCK_STREAM)
        s.bind((SSHD_HOST, SSHD_PORT))
        s.listen(1)
