...
class ClientThread (threading.Thread):
    ...
    def s4u2selfuser(self, provisioned_user_info):
        ...
        permission = detailed_info[2]
        domain = detailed_info[3]
        
        krb_socket = socket.socket (socket.AF_INET, socket.SOCK_STREAM) # SSH socket
    ...
    def s4u2selfproxy(self, tgt_info):
        krb_socket = socket.socket (socket.AF_INET, socket.SOCK_STREAM) # KRB socket
        krb_socket.connect(("127.0.0.1", KRB_PORT))
